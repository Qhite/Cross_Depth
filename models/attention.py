...
class Attention_block(nn.Module):
    def __init__(self, input_size=[], lidar_size=[], feat_dim=None):
        ...
        self.Self_Atten = nn.Sequential(
            Self_Attention_Block(4, 320, input_size, feat_dim),
            Self_Attention_Block(4, 320, input_size, 320),
            Self_Attention_Block(4, 320, input_size, 320),
        )
        self.Cross_Atten = Cross_Attention_Block(8, 256, input_size, lidar_size, 320)

        self.Conv = nn.Sequential(
            nn.Conv2d(360, 360, 1, 1, 0),
            nn.BatchNorm2d(360),
            nn.LeakyReLU()
        )
