...
# Dataload
test_loader = dataloader.getTest_Data(batch_size=1, lidar_size=cfg.lidar)

tools.show_cfg(cfg, model, True)

# Loss Funtions
Loss = models.Losses(m_cfg, cfg).to(device=cfg.device)
psu_lidar = tools.pesudo_lidar(cfg.lidar)
...
if __name__ == "__main__":
    ...
    path = f"/root/output/{cfg.train.tag}-nyu-eval"
    tools.visualization(test_loader, model, path, cfg.device, cfg.dataset, cfg.lidar)
